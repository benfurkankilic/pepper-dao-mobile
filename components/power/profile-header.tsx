...
function getNextRankTasks(profile: Profile, nextRank: Rank): Array<RankTask> {
  ...
  if (requirements.requiresEngagedProposal) {
    tasks.push({
      ...
      completed: profile.has_engaged_proposal,
    });
  }
  ...
  if (requirements.requiresPassedProposal) {
    tasks.push({
      ...
      completed: profile.has_passed_proposal,
    });
  }
...
export function ProfileHeader({ profile }: ProfileHeaderProps) {
  ...
  return (
    <Card variant="dark" className="p-4">
      ...
      {/* Progress Bar */}
      <View className="mt-3 h-4 w-full overflow-hidden border-2 border-white/20 bg-[#333333]">
        <View
